...
func main() {
	...
	if err := t.Load(data,
		tree.WithIDFunc(func(c Category) int { return c.ID }),
		tree.WithParentIDFunc(func(c Category) int { return c.ParentID }),
	); err != nil {
		panic(err)
	}
	...
	rootNode := t.ToTree(1)
	// rootNode now contains a complete tree structure with children nodes
...
3. Traversing paths in the tree:
